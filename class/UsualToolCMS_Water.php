<?php
class UsualToolCMSWater{
    /*
    * UsualToolCMS2.0版文字及图片水印函数,升级水印函数请下载最新版UsualToolCMS从class目录中获得
    * 有任何问题欢迎交流,站长圈子qq群:583610949
    * 作者:黄豆 更新时间:2018/06/03
    * 参照部分水印函数进行更新修正,请保留作者信息
    * 参数解释:
    * type 类型 值为:text或image
    * photo 需要加水印的图片
    * content 水印内容 当type为text时该值为文字,当type为image时该值为水印图片地址
    * waterpos 水印位置 0为随机位置 1为顶端居左，2为顶端居中，3为顶端居右，4为中部居左，5为中部居中，6为中部居右，7为底端居左，8为底端居中，9为底端居右
    * fontcolor 文字颜色 可不设置为默认 图片水印忽略
    * fontsize 文字大小 可不设置为默认 图片水印忽略
    * 函数调用示例:
    //UsualToolCMSWater::markWater("text","1.jpg","站点:cms.usualtool.com",5,"#B5B5B5","14");
    //UsualToolCMSWater::markWater("image","1.jpg","water.png",8);
    */
    ...
    static function markWater($type,$photo,$content,$waterpos,$fontcolor='#B5B5B5',$fontsize='14'){
        $fontcolor=UsualToolCMSWater::hex2rgb($fontcolor);
