...
class tree{
    ...
    public function get_tree3($myid, $str, $sid = 0, $adds = '', $str_group = ''){
        $number=1;
        $child = $this->get_child($myid);
        if(is_array($child)){
            $total = count($child);
        foreach($child as $id=>$value){
            ...
            $this->ret .= $nstr;
            $nbsp = $this->nbsp;
            $this->get_tree3($id, $str, $sid, $adds.$k.$nbsp,$str_group);
            $number++;
        }
        }
        return $this->ret;
    ...
    function get_treeview($myid,$effected_id='USUALTOOLCMS',$str="<span class='file'>\$name</span>", $str2="<span class='folder'>\$name</span>" ,$showlevel = 0 ,$style='filetree' , $currentlevel = 1,$recursion=FALSE) {
        $child = $this->get_child($myid);
